...
async function sendWhatsappOTPV2(phone, otp) {
  const fetch = (await import("node-fetch")).default;
...
async function sendOTPV2(phone, role) {
  const docId = `${phone}`;
  const userRef = db.collection("login_users").doc(docId);
  const userSnap = await userRef.get();
  ...
  const otp = generateOTP();
  await userRef.update({ otp });
  await sendWhatsappOTPV2(phone, otp);

  return otp;
}

async function verifyOTPV2(phone, role, inputOtp) {
  const docId = `${phone}`;
  const userRef = db.collection("login_users").doc(docId);
  const userSnap = await userRef.get();
